...
```
Widgets/
├── registry.json                    # Widget Registry JSON (the source of truth)
├── schema/
...
| `npm run validate` | Validate `registry.json` against the JSON schema |
...
Publish `registry.json` and `dist/index.js` to a publicly accessible URL. For example:

```
https://your-cdn.example.com/widgets/registry.json
https://your-cdn.example.com/widgets/dist/index.js
```

Update the `content.endpoint` in `registry.json` to point to your actual API endpoint.
...
3. Add a new registry with:
   ...
   - **Registry URL**: `https://your-cdn.example.com/widgets/registry.json`
...
3. Add the widget definition to `registry.json` under the `widgets` array
